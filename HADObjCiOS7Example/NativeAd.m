...
-(void)HADNativeAdDidLoad:(HADNativeAd *)nativeAd {
    [self.bannerMediaView loadHADBanner:nativeAd animated:NO completion:^(NSError * error, UIImage * image) {
        if (!error) {
    ...
    [self.iconMediaView loadHADIcon:nativeAd animated:NO completion:^(NSError * error, UIImage * image) {
        if (!error) {
